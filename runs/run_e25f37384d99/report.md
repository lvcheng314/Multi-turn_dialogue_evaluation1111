# 飞毛腿外呼测评报告2026/06/05/0013

## 评测摘要

- 报告名称: 飞毛腿外呼测评报告2026/06/05/0013
...
- 任务结果 Outcome（平均 26.40 / 30 分）: 检查任务目标是否达成，以及关键流程是否覆盖完整。
...
- fm_contract_effective_003: 总分 66.40 / 100，最低维度 工具轨迹 Trace（0.00 / 30 分）。
...
| --- | ---: | --- | ---: | ---: | ---: | ---: |
| fm_contract_effective_003 | 66.40 / 100 | 复核 | 26.40 / 30 | 0.00 / 30 | 20.00 / 20 | 20.00 / 20 |
...
- 场景目标: 确认任务接受状态。
- 总分: 66.40 / 100
- 分项: 任务结果 Outcome（26.40 / 30 分），工具轨迹 Trace（0.00 / 30 分），安全合规 Safety（20.00 / 20 分），话术质量 Text（20.00 / 20 分）
- 结论: 复核
...
- 任务结果 Outcome（26.40 / 30 分）
  - 状态级证据: 规则 outcome.final_state_and_flow_coverage; 最终状态: accepted; 预期状态: accepted; 必需流程覆盖 4/5。
...
- 第 2 轮 用户: 我是本人。合同到底什么时候生效？今天就算吗？
- 第 3 轮 数字人: 告知骑手今天飞毛腿合同已生效，并询问是否可以开始配送。如果未完成配送，合同和派单可能受影响。
- 第 4 轮 用户: 明白了。午晚高峰几点到几点？
- 第 5 轮 数字人: 午高峰11点到1点，晚高峰5点到7点。今天能正常上线配送吗？配送时并提醒注意安全。
- 第 6 轮 用户: 可以，今天会正常配送。有什么要注意的规则？
- 第 7 轮 数字人: 报名名额按排名分配，完成率越高排名越靠前。减少拒单、取消和超时，有问题随时联系站点。
- 第 8 轮 用户: 好的，清楚了，今天正常配送。
- 第 9 轮 数字人: 好的，那今天先确认到这里。
- 第 10 轮 用户: 好，再见。
- 第 11 轮 数字人: 感谢接听，祝您跑单顺利，再见！
