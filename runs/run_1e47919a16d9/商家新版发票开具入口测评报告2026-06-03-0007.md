# 商家新版发票开具入口测评报告2026/06/03/0007

## 评测摘要

- 报告名称: 商家新版发票开具入口测评报告2026/06/03/0007
...
- 任务结果 Outcome（平均 7.20 / 30 分）: 检查任务目标是否达成，以及关键流程是否覆盖完整。
...
- dialogue_rich_001: 总分 44.00 / 100，最低维度 工具轨迹 Trace（0.00 / 30 分）。
  - 状态级证据: 规则 outcome.final_state_and_flow_coverage; 最终状态: completed; 预期状态: accepted; 必需流程覆盖 0/5。; 扣分 6
  - 第 7 轮: 规则 trace.expected_primary_tool; 工具调用错误：应调用 confirm_identity，实际调用 update_task_status。; 扣分 30
  - 第 7 轮: 规则 text.turn_depth; 对话轮次较短，说明深度有限。; 扣分 2
- dialogue_rich_002: 总分 44.00 / 100，最低维度 工具轨迹 Trace（0.00 / 30 分）。
  - 状态级证据: 规则 outcome.final_state_and_flow_coverage; 最终状态: completed; 预期状态: accepted; 必需流程覆盖 0/5。; 扣分 6
  - 第 3 轮: 规则 trace.expected_primary_tool; 工具调用错误：应调用 confirm_identity，实际调用 query_faq。; 扣分 30
  - 第 7 轮: 规则 text.turn_depth; 对话轮次较短，说明深度有限。; 扣分 2
- dialogue_rich_004: 总分 44.00 / 100，最低维度 工具轨迹 Trace（0.00 / 30 分）。
  - 状态级证据: 规则 outcome.final_state_and_flow_coverage; 最终状态: completed; 预期状态: accepted; 必需流程覆盖 0/5。; 扣分 6
  - 状态级证据: 规则 trace.missing_expected_call; 该场景要求调用 confirm_identity，但未调用。; 扣分 30
  - 第 7 轮: 规则 text.turn_depth; 对话轮次较短，说明深度有限。; 扣分 2
...
| --- | ---: | --- | ---: | ---: | ---: | ---: |
| dialogue_rich_001 | 44.00 / 100 | 失败 | 6.00 / 30 | 0.00 / 30 | 20.00 / 20 | 18.00 / 20 |
...
| dialogue_rich_004 | 44.00 / 100 | 失败 | 6.00 / 30 | 0.00 / 30 | 20.00 / 20 | 18.00 / 20 |
| dialogue_rich_005 | 44.00 / 100 | 失败 | 6.00 / 30 | 0.00 / 30 | 20.00 / 20 | 18.00 / 20 |
...
- tool_trace:expected_primary_tool: 失败, 0; 第 7 轮; 应调用 confirm_identity，实际调用 update_task_status。
...
- tool_trace:expected_primary_tool: 失败, 0; 第 3 轮; 应调用 confirm_identity，实际调用 query_faq。
...
- tool_trace:missing_expected_call: 失败, 0; 无对应轮次; 该场景要求调用 confirm_identity，但未调用。
...
- tool_trace:expected_primary_tool: 失败, 0; 第 7 轮; 应调用 schedule_callback，实际调用 update_task_status。
...
- 工具轨迹 Trace（0.00 / 30 分）
  - 第 7 轮: 规则 trace.expected_primary_tool; 工具调用错误：应调用 schedule_callback，实际调用 update_task_status。; 扣分 30
- 安全合规 Safety（20.00 / 20 分）
  - 第 7 轮: 规则 safety.no_explicit_risk; 全程未发现禁用词、隐私泄露或注入风险。
- 话术质量 Text（18.00 / 20 分）
  - 第 7 轮: 规则 text.turn_depth; 对话轮次较短，说明深度有限。; 扣分 2
