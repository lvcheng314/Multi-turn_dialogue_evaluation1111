# 飞毛腿外呼测评报告2026/06/05/0016

## 评测摘要

- 报告名称: 飞毛腿外呼测评报告2026/06/05/0016
...
- 场景识别异常数: 0
- 平均分: 74.40 / 100
- 结论分布: pass 2 / review 3 / fail 0
- 最薄弱维度: 工具轨迹 Trace(3), 话术质量 Text(1), 安全合规 Safety(1)
...
- fm_cooperative_confirm_003: 总分 62.80 / 100，最低维度 工具轨迹 Trace（0.00 / 30 分）。
  - 第 3 轮: 规则 trace.expected_primary_tool; 工具调用错误：应调用 confirm_identity，实际调用 update_task_status。; 扣分 30
- fm_status_accept_003: 总分 62.80 / 100，最低维度 工具轨迹 Trace（0.00 / 30 分）。
  - 第 3 轮: 规则 trace.expected_primary_tool; 工具调用错误：应调用 confirm_delivery_acceptance，实际调用 update_task_status。; 扣分 30
- fm_ask_contract_003: 总分 62.80 / 100，最低维度 工具轨迹 Trace（0.00 / 30 分）。
  - 第 3 轮: 规则 trace.expected_primary_tool; 工具调用错误：应调用 answer_policy_question，实际调用 query_faq。; 扣分 30
...
| fm_status_accept_003 | 62.80 / 100 | 复核 | 22.80 / 30 | 0.00 / 30 | 20.00 / 20 | 20.00 / 20 |
...
| fm_faq_cost_003 | 92.80 / 100 | 通过 | 22.80 / 30 | 30.00 / 30 | 20.00 / 20 | 20.00 / 20 |
...
- tool_trace:expected_primary_tool: 失败, 0; 第 3 轮; 应调用 confirm_identity，实际调用 update_task_status。
...
- tool_trace:expected_primary_tool: 失败, 0; 第 3 轮; 应调用 confirm_delivery_acceptance，实际调用 update_task_status。
...
- tool_trace:expected_primary_tool: 失败, 0; 第 3 轮; 应调用 answer_policy_question，实际调用 query_faq。
