# 飞毛腿外呼测评报告2026/06/05/0011

## 评测摘要

- 报告名称: 飞毛腿外呼测评报告2026/06/05/0011
...
- 场景识别异常数: 0
- 平均分: 67.80 / 100
- 结论分布: pass 5 / review 5 / fail 4
- 最薄弱维度: 任务结果 Outcome(9), 工具轨迹 Trace(4), 话术质量 Text(1)

## 维度说明

- 任务结果 Outcome（平均 9.44 / 30 分）: 检查任务目标是否达成，以及关键流程是否覆盖完整。
- 工具轨迹 Trace（平均 21.43 / 30 分）: 检查工具调用是否正确、参数是否匹配、顺序是否合理。
...
| --- | ---: | --- | ---: | ---: | ---: | ---: |
| fm_ask_quota_rule_002 | 81.20 / 100 | 通过 | 13.20 / 30 | 30.00 / 30 | 20.00 / 20 | 18.00 / 20 |
...
- 场景目标: 说明关键时间点并促成继续执行。
- 总分: 81.20 / 100
- 分项: 任务结果 Outcome（13.20 / 30 分），工具轨迹 Trace（30.00 / 30 分），安全合规 Safety（20.00 / 20 分），话术质量 Text（18.00 / 20 分）
- 结论: 通过
...
| --- | --- | ---: | --- | --- |
| fm_reject_bypass_rules_002 | s12_faq_cost | 0.65 | 用户追问内部名额分配和排名算法，本质是关心收益和规则背后的成本/收益问题，属于费用收益类FAQ场景。虽然用户最终未明确拒绝，但对话核心是追问规则细节，而非配合确认或直接接受任务。 | 未纳入量化评分，建议人工复核 |
